...
Needed packages and initializing h2o and the common seed:
...
```{r , include=TRUE, cache=TRUE, message=F, results=T, echo=F, fig.align='center'}
...
```{r , include=T, cache=TRUE, message=F, results=F, echo=F}
h2o.varimp_plot(best_rf)
```

###### XGBOOST

```{r, include=TRUE, cache=TRUE, message=F, results=F, echo=F}
...
In the following code snippet I am training the two random forest models as described in the task above.
...
###### mtries=2
```{r varimp_plots, fig.width=8,fig.height=4, fig.align='center', warning = FALSE, message = FALSE, echo=F}
h2o.varimp_plot(rf_2)
```

###### mtries=10
```{r varimp_plots22, fig.width=8,fig.height=4, fig.align='center', warning = FALSE, message = FALSE, echo=F}
...
In the following code snippet I am training the two GBM models as described in the task above.
...
In the following code snippet I am cutting the data as described above. Additionally I am defining the y and X variables.
...
I decided to use a GBM model as a benchmark model, which has a cross-validated AUC of 0.5682 on the validation set.

```{r benchmark_train , include=TRUE, cache=TRUE, message=F, results=F, echo=T, }
simple_gbm <- h2o.gbm(
  x = X, y = y,
  model_id = "simple_gbm",
  training_frame = data_train,
...
I made the decision to train the following models: 
...
The training of these models are the following: 
...
In the table below we can see the cross-validated AUC on the validation set for all of the models. (Note that the cross-validated AUC of for the baseline GBM model was 0.5682 on the validation set.).

```{r eval3, echo = FALSE , results = "asis", warning = FALSE, message = FALSE, cache=F }
...
colnames(aucs_df) <- 'AUC (validation)'
rownames(aucs_df) <- c('Lasso', 'RF', 'DL', 'GBM')

kbl(aucs_df, digits = 4)  %>% kable_styling(bootstrap_options = "striped", full_width = F, position = "center")
```

#### e. How large are the correlations of predicted scores of the validation set produced by the base learners?

To visualize the correlation between the models we can plot heatmap showing the correlation matrix. It is interesting to see, that there is a great correlation between lasso, random forest and deep learning, however the GBM models seems to be different. (It had a poorer AUC performance as well)

```{r corplot, include=TRUE, cache=T, message=F, results=T,echo=F, fig.align='center'}
h2o.model_correlation_heatmap(my_models, data_valid)
```

#### f. Create a stacked ensemble model from the base learners.

Let's combine these models by training a meta learner model on top of them. First I am training with a default GLM meta learner, then I am using a GBM meta lerner in the following code snippet.
...
The AUC on the validation set turned out to be 0.5794 for the glm meta learner and 0.5424 for the GBM meta learner. The GLM meta learner under performed the other models, but the default glm meta learner improved slightly. 
...
According to the results before, the glm meta learner turned out to be the best model. I have evaluated it on the test set and the AUC turned out to be 0.5907 which is better than on the validation set. It means that to model is robust enough.
...
From the following plot we can see that the best RF models turned out to be the ones with small sample rate, with maximum depth 10. We can see that after 300 trees almost all the options reached the optimal state.

```{r , include=TRUE, cache=TRUE, message=F, results=F, echo=F, fig.align='center'}
rf_summary_plot
```

#### Summary plot for GBM tuning

From the following plot we can see that with GBM models overfitting is really frequent, especially whey using high learning rate, and deep trees. Also we can see that with small learning rate, it is just the matter of time (number of trees) when we reach great performing models. 
```{r , include=TRUE, cache=TRUE, message=F, results=F, echo=F, fig.align='center'}
gbm_summary_plot
```

...
Needed packages and initalizin h2o and the common seed:
...
- It worth to mention that WeekofPurchase is numeric and we might make it as a factor as well. However I decided to keep it as it is as it values are between 254 to 278 indicating that it has some time series feature in it (254 should be truly the furthers from 278).
...
According to the previous table, the best model is a GBM model. Next, I calculated the AUC on the test set which is 0.8679 which is still high. On the other hand, it went down a bit which means, that the CV AUC metrics showed before are indeed very similar compared to this change. Also, we can plot the ROC curve as a visual representation of the AUC(area under the (ROC)curve).
...
Up next we can see the variable importance plots of the models. We can see that they are very similar as the LoyalCH variable is outstandingly important everywhere. In the following places, there are some changes, but we can say that both models are very similar. 
...
```{r data2, include=FALSE, cache=TRUE, message=F, results=F}
...
```{r rf_train, include=TRUE, cache=TRUE, message=F, results=F}
##y <- "log_salary"
#X <- setdiff(names(h2o_data), y)
#
#rf_params <- list(
...
```{r rf_grid, include=FALSE, cache=TRUE, message=F, results=F}
#rf_2_performance_summary <- h2o.getGrid(rf_grid_2@grid_id, "mae")@summary_table %>%
#  as_tibble() %>%
#  mutate(across(c("mae", names(rf_params)), as.numeric))
#
#rf_2_performance_summary_plot <- ggplot(rf_performance_summary, aes(ntrees, mae, color = factor(mtries))) +
#  geom_line() +
#  facet_grid(max_depth ~ sample_rate, labeller = label_both) +
#  theme(legend.position = "bottom") +
#  labs(color = "mtry")
#
#rf_10_performance_summary <- h2o.getGrid(rf_grid_10@grid_id, "mae")@summary_table %>%
#  as_tibble() %>%
#  mutate(across(c("mae", names(rf_params)), as.numeric))
#
#rf_10_performance_summary_plot <- ggplot(rf_performance_summary, aes(ntrees, mae, color = factor(mtries))) +
#  geom_line() +
#  facet_grid(max_depth ~ sample_rate, labeller = label_both) +
#  theme(legend.position = "bottom") +
#  labs(color = "mtry")
...
```{r psp, fig.width=8,fig.height=4, fig.align='center', echo = FALSE , results = 'asis', warning = FALSE, message = FALSE}
#ggarrange(rf_2_performance_summary_plot, rf_10_performance_summary_plot, ncol=2)
```

```{r varimp_plots, fig.width=8,fig.height=4, fig.align='center', echo = FALSE , results = 'asis', warning = FALSE, message = FALSE}
#ggarrange(h2o.varimp_plot(best_rf_2),h2o.varimp_plot(best_rf_10), ncol=2)
```
...
```{r gbm_train, include=TRUE, cache=TRUE, message=F, results=F}
#gbm_params <- list(
#  ntrees = 500,
#  learn_rate = 0.1, 
#  max_depth = 5,
...
```{r gbm_grid, include=FALSE, cache=TRUE, message=F, results=F}
#rf_2_performance_summary <- h2o.getGrid(rf_grid_2@grid_id, "mae")@summary_table %>%
#  as_tibble() %>%
#  mutate(across(c("mae", names(rf_params)), as.numeric))
#
...
```{r psp_gbm, fig.width=8,fig.height=4, fig.align='center', echo = FALSE , results = 'asis', warning = FALSE, message = FALSE}
#ggarrange(rf_2_performance_summary_plot, rf_10_performance_summary_plot, ncol=2)
```

```{r varimp_plots_gbm, fig.width=8,fig.height=4, fig.align='center', echo = FALSE , results = 'asis', warning = FALSE, message = FALSE}
#ggarrange(h2o.varimp_plot(gbm_01),h2o.varimp_plot(gbm_01), ncol=2)
```

...
In the following code snippet I am using the kmeans function to do kmeans clustering with 2 clusters and getting the class labels. I am setting the nstarts parameter to 5 in order to get the best model out of 5 attempt. This is needed as kmeans clustering is not deterministic, therefore it can end up in strange outcomes.  
...
Next I am plotting the observations colored by clusters in the space of urban population and assaults. 
...
In the following code snippet i am performing PCA on the original variables and I am getting the firsth three principal components as I would like to experiment with a 3D plotting as well. 
...
As the exercise as for it the following plot uses the first two principal components and shows the clusters on the plain defined by PC1 and PC2. We can observe that the clusters are separated along PC1 similarly like the previous plot when we had the separation along the assault axis. On the other hand we still have a broad variance in each cluster among the PCA axis. 
...
In the end, I was curious about the 3D case using the first three PCs. In the following 3D interactive plot we can see that both three variables has great variance in the whole data, meaning they contain great amount of information. 
...
In the following code snippet I am performing PCA on the dataeset with scaling features.
...
By using the fviz_pca_ind function i plotted the data points on the plain of the first two principal components. On the figure it is clear, that the healty and non-healthy observations are greatly separated among the first PC. 
...
From the previous plot we can see that PC1 metters a lot regarding the separation of the dataset into two groups. By the fviz_contrib function we can visualize the top 5 loadings for the first PC. These are the ones, that are the most important variables in PC1 (having the largest weight in the linear combination of all original variables). We could have used the rotation matrix also, to find the most important variables, the fviz_contrib function gives the same. 
...
We can see the two most important features (V502, V589) and we can use them, to plot the observations in the coordinate system defined by these two original features. On the plot it is clear, the these toowo variables are truly important as the data points are well separated on this plain already. However it is important to highlight that on the PC1-PC2 plain the seperation was more straightforward.
